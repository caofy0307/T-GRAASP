...
from torch_geometric.utils import remove_self_loops, to_dense_batch, to_dense_adj, dense_to_sparse, negative_sampling, to_undirected
import numpy 
...
class SGEDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for i in self.id:
            ...
            pos_barcode = pos['V1'].values       
            if not all(node_id == node_barcode) & all(node_barcode == pos_barcode):
                try:
            ...
            adj = (dst < 150.).int()
            category_2d = y[j][:, numpy.newaxis]
            matrix = numpy.where(category_2d == category_2d.T, 0, 1)
            adj1 = adj * matrix
            edge_index, _ = dense_to_sparse(adj)
            edge_index, _ = remove_self_loops(edge_index)
            edge_index = to_undirected(edge_index)
            edge_index1, weight = dense_to_sparse(adj1)
            edge_index1, weight = remove_self_loops(edge_index1, weight)
            edge_index1, weight = to_undirected(edge_index1, weight)            
            edge_weight = weight/2
            g = Data(x=x[j], y=y[j].long(), edge_index=edge_index, edge_index1 = edge_index1, edge_weight=edge_weight)
